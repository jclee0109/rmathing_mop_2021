...
#데이터베이스에서 2021 fall 삭제하는 함수
def del_2021_fall(request):
    subject_2021_fall = SubjectInfo.objects.filter(year = 2021, session = 'fall')
    subject_2021_fall.delete.all()
    return redirect('timetable:mytable', user_id=request.user.id)
...
def data_save(request):
    ...
    for i in range(len(data_pd) - 1):
        ...
        #2021년 fall 데이터베이스 추가 -> 수정필요
        subject.year = 2021
        subject.session = 'fall'
        subject.save()
