...
def main(request):
    ...
    so = request.GET.get('so', 'default')  # 정렬기준

    if so == 'select':
        subject_list = SubjectInfo.objects.order_by('-select_person', 'name')
    else:
    ...
    context = {'subject_list': page_obj, 'page': page, 'kw': kw, 'so': so}
...
def mytable(request, user_id):
    ...
    if so == 'select':
        qs = SubjectInfo.objects.all().order_by('-select_person', 'name')
    elif so == 'name':
        qs = SubjectInfo.objects.all().order_by('name')
    else:
